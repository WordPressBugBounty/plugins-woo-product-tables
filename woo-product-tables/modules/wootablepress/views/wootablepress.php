...
class WootablepressViewWtbp extends ViewWtbp {
	...
	public function loadAssets() {
		...
		FrameWtbp::_()->addScript( 'wtbp.buttonshtml', $this->getModule()->getModPath() . 'js/dt/buttons.html5.min.js' );
		...
		if ( isset( $options['locale_sorting'] ) && isset( $options['locale_sorting']['value'] ) && ! empty( $options['locale_sorting']['value'] ) ) {
			$locale = get_locale();
			if ($locale && strlen($locale) > 1) {
				FrameWtbp::_()->addJSVar( 'wtbp.core.tables.js', 'wtbp_use_locale_sorting', str_replace('_', '-', $locale) );
			}
		}
	...
	public function getProductContent( $productIds, $tableSettings, $preview = true, &$page = array() ) {
			...
			if ( empty( $settings['pre_sorting'] ) ) {
				...
				switch ( $settings['pre_sorting'] ) {
					...
					case 'menu_order':
						...
						break;
					case 'categories':
						add_filter( 'posts_clauses', array( $this, 'addOrderByCategories' ), 99999 );
						break;
				}
			}
		}
		...
		if ( $isPro ) {
			DispatcherWtbp::doAction( 'beforeProductsTableLoopPro', $settings );
		}
		...
		if ( $isPro ) {
			DispatcherWtbp::doAction( 'afterProductsTableLoopPro', $settings );
		}
		if ( $isPage || ! $frontend ) {
