...
abstract class ModelWtbp extends BaseObjectWtbp {
	...
	public function getFromTbl( $params = array() ) {
		...
		$table = FrameWtbp::_()->getTable( $tbl );
	...
	protected function _buildQuery( $table = null ) {
		if (!$table) {
			$table = FrameWtbp::_()->getTable( $this->_tbl );
		}
	...
	public function removeGroup( $ids ) {
		...
		if (!empty($ids)) {
			if (FrameWtbp::_()->getTable( $this->_tbl )->delete(array('additionalCondition' => 'id IN (' . implode(',', $ids) . ')'))) {
				return true;
			} else {
				$this->pushError (esc_html__('Database error detected', 'woo-product-tables'));
			}
		} else {
	...
	public function delete( $params = array() ) {
		if (FrameWtbp::_()->getTable( $this->_tbl )->delete( $params )) {
			return true;
		} else {
			$this->pushError (esc_html__('Database error detected', 'woo-product-tables'));
		}
	...
	public function insert( $data ) {
		...
		$this->pushError(FrameWtbp::_()->getTable( $this->_tbl )->getErrors());
		return false;
	}
	public function updateById( $data, $id = 0 ) {
		...
		if ($id) {
			...
			$this->pushError(esc_html__('Empty or invalid ID', 'woo-product-tables'));
		}
		return false;
	}
	public function update( $data, $where ) {
		$data = $this->_dataSave($data, true);
		if (FrameWtbp::_()->getTable( $this->_tbl )->update( $data, $where )) {
			return true;
		}
		$this->pushError(FrameWtbp::_()->getTable( $this->_tbl )->getErrors());
		return false;
