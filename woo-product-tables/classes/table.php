...
abstract class TableWtbp {
			...
			if ($id) {
				...
				foreach ($data as $field) {
					...
					foreach ($field as $k => $v) {
						if (isset($this->_fields[$k])) {
							$row[$k] = toeCreateObjWtbp('FieldWtbp', array(
									$this->_fields[$k]->name,
									$this->_fields[$k]->html,
									$this->_fields[$k]->type,
	...
	public function getTable( $transform = false ) {
		if ($transform) {
			return DbWtbp::prepareQuery($this->_table);
		} else {
			return $this->_table;
		}
	}
	...
	public function get( $fields = '*', $where = '', $tables = '', $return = 'all' ) {
		...
		return DbWtbp::get($query, $return);
	}
	public function store( $data, $method = 'INSERT', $where = '' ) {
		$this->_clearErrors();
		$method = strtoupper($method);
		if ($this->_escape) {
			$data = DbWtbp::escape($data);
		}
		$query = '';
		switch ($method) {
			case 'INSERT':
		...
		if (DbWtbp::query($query)) {
			if ('INSERT' == $method) {
				return DbWtbp::lastID();
			} else {
				return true;
			}
		} else {
			$this->_addError(WTBP_TEST_MODE ? DbWtbp::getError() : esc_html__('Database error. Please contact your developer.', 'woo-product-tables'));
		}
		return false;
	}
	...
	/**
	 * Add new FieldWtbpWtbp for children table (@see class field)
	 *
	...
	protected function _addField( $name, $html = 'text', $type = 'other', $default = '', $label = '', $maxlen = 0, $dbAdapt = '', $htmlAdapt = '', $description = '' ) {
		$this->_fields[$name] = toeCreateObjWtbp('FieldWtbp', array($name, $html, $type, $default, $label, $maxlen, $dbAdapt, $htmlAdapt, $description));
		return $this;
	}
	/**
	...
	public function exists( $value, $field = '' ) {
		...
		return DbWtbp::get('SELECT ' . $this->_id . ' FROM ' . $this->_table . ' WHERE ' . $field . ' = "' . $value . '"', 'one');
	}
			...
			switch ($f->type) {
				case 'date':
					if ( ( '0000-00-00' == $d[$key] ) || empty($d[$key]) ) {
						...
						$d[$key] = gmdate(WTBP_DATE_FORMAT, DbWtbp::dateToTime($d[$key]));
					}
					break;
				case 'int':
				case 'tinyint':
	...
	public function getLastInsertID() {
		return DbWtbp::get('SELECT MAX(' . $this->_id . ') FROM ' . $this->_table, 'one');
	}
