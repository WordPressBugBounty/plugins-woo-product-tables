...
class UtilsWtbp {
	...
	public static function getFilesList( $path ) {
		...
		if (is_dir($path)) {
			...
			while ( ( $file = readdir($dirHandle) ) !== false ) {
				if (( '.' != $file ) && ( '..' != $file ) && ( '.svn' != $file ) && is_file($path . DS . $file)) {
					$files[] = $file;
				}
			}
		}
	...
	public static function isThisCommercialEdition() {
		foreach (FrameWtbp::_()->getModules() as $m) {
	...
	public static function isPluginsPage() {
		return ( basename(ReqWtbp::getVar('SCRIPT_NAME', 'server')) === 'plugins.php' );
	}
	...
	/**
	 * Parse worwtbpess post/page/custom post type content for images and return it's IDs if there are images
	 *
