...
class ResponseWtbp {
	...
	public function ajaxExec( $forceAjax = false ) {
		$isAjax = $this->isAjax();
		$redirect = ReqWtbp::getVar('redirect');
		if (count($this->errors) > 0) {
			$this->error = true;
		}
		if ($isAjax || $forceAjax) {
			HtmlWtbp::echoEscapedHtml(jsonEncodeUTFnormalWtbp($this));
			exit();
		}
		return $this;
	}
	public function mainRedirect( $redirectUrl = '' ) {
		$redirectUrl = empty($redirectUrl) ? WTBP_SITE_URL : $redirectUrl;
		$redirectData = array();
		if (!empty($this->errors)) {
			$redirectData['wtbpErrors'] = $this->errors;
		}
		if (!empty($this->messages)) {
			$redirectData['wtbpMsgs'] = $this->messages;
		}
		return redirectWtbp( $redirectUrl . ( strpos($redirectUrl, '?') ? '&' : '?' ) . http_build_query($redirectData) );
	}
	...
	/**
	 * Alias for ResponseWtbp::addError, 
	 *
	 * @see addError method
	 */
